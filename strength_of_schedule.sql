...
-- SoS average is 0.5, higher is harder schedule.
with season_games as (
	select *
	from data.games g
	where g.season = (select max(tm.season) from data.time_map tm)
	and g.day < least((select max(tm.day) from data.time_map tm where tm.season=g.season) , 99)
),
record as (
	select 	t.team_id, 
			t.nickname,
